...
팀 전체 기능 중 지원서 접수·심사·투표, 회원 인증/가입, 이메일 인증, 회비 관리, 코딩테스트 문제, 자료실, 뉴스레터, 댓글·신고, 활동보고서는 다른 팀원이 담당했습니다.

## 기술적으로 해결한 문제

### 1. 모임 목록 조회의 N+1 쿼리 제거

**문제**

모임 목록 응답 한 건에는 (a) 작성자 이름·기수, (b) 내 투표 상태, (c) 참석/불참/미응답 집계가 모두 필요합니다. 모임마다 이 셋을 조회하면 목록 크기 N에 비례해 쿼리가 늘어납니다. 특히 회의·MT 유형은 생성 시점에 전체 활동 회원을 `NOT_RESPONDED`로 일괄 생성하기 때문에, **모임 하나당 참석 레코드가 회원 수만큼 존재**합니다.

**원인 분석**

작성자는 FK 연관이 아니라 `Gathering.authorId`(Long)로만 들고 있어 JPA의 fetch join으로 해결할 수 없었고, 참석 집계는 `gathering_attendance`를 모임별로 다시 조회해야 하는 구조였습니다. 그대로 두면 **`1(목록) + N(작성자) + N(참석)`** 형태가 됩니다.

**해결**

목록을 먼저 조회한 뒤, 작성자 ID 집합과 모임 ID 집합으로 각각 한 번씩만 일괄 조회하고 메모리에서 `Map`으로 조립하도록 재작성했습니다.
...
첨부파일도 같은 이유로 목록 응답에서 제외하고 상세 조회에서만 내려주도록 API를 분리했습니다. 또 단건 조회·수정 응답에서는 참석 레코드를 전부 로딩하지 않고 상태별 `COUNT` 3회로 요약을 계산하도록, 목록용과 단건용 집계 메서드를 나눴습니다.
...
| 목록 응답의 첨부파일 조회 | 모임당 1회 추가 | 목록에서 제외 (상세에서만 조회) |
| 단건 조회 시 참석 레코드 | 전체 로딩 후 집계 | 상태별 `COUNT` 3회 |

> 쿼리 수는 코드에서 도출한 값입니다. 실제 응답시간·처리량은 측정하지 않았습니다.

### 2. multipart 요청의 JSON 파트가 415로 거부되던 문제

**문제**

모임 등록 API는 모임 정보(JSON)와 첨부파일을 한 번에 받는 `multipart/form-data` 엔드포인트입니다. 그런데 클라이언트가 JSON 파트에 `Content-Type` 헤더를 붙이지 않고 보내면 서버가 **`415 Unsupported Media Type`** 으로 요청을 거부했습니다.

**원인 분석**

`@RequestPart`로 받는 파트는 파트별 `Content-Type`을 보고 메시지 컨버터를 선택합니다. 클라이언트가 파트의 타입을 명시하지 않으면 `application/octet-stream`으로 들어오는데, Jackson 컨버터의 기본 지원 타입 목록에는 이 타입이 없어 "처리할 컨버터가 없음"으로 판정됩니다. **본문의 JSON 자체는 정상인데 타입 협상 단계에서 막히는** 문제였습니다.

**해결**

컨트롤러마다 우회하는 대신, Jackson 컨버터의 지원 미디어 타입에 `application/octet-stream`을 추가하는 전역 설정을 넣었습니다.
...
| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| JSON 파트에 `Content-Type` 미지정 시 | `415 Unsupported Media Type` | 정상 처리 (JSON 역직렬화) |
| 클라이언트 대응 부담 | 파트별 `Content-Type` 수동 지정 필요 | 불필요 |
| 적용 범위 | — | 전역 (multipart를 쓰는 모든 엔드포인트) |

### 3. S3 저장소와 DB 트랜잭션 사이의 정합성 확보

**문제**

첨부파일의 실제 바이트는 S3에, 메타데이터는 MySQL에 나눠 저장합니다. 두 저장소는 하나의 트랜잭션으로 묶이지 않기 때문에 **양방향으로 불일치**가 생길 수 있었습니다.

1. S3 업로드는 성공했는데 뒤이은 DB 저장이 실패 → 아무도 참조하지 않는 **고아 객체**가 남음
2. 트랜잭션 안에서 DB 삭제와 S3 삭제를 함께 하다가 롤백 → DB에는 레코드가 살아 있는데 실제 파일은 이미 지워진 **유령 레코드** 발생

**원인 분석**

원인은 "외부 저장소 호출을 트랜잭션 경계와 무관하게 호출한다"는 점 하나로 좁혀졌습니다. 업로드는 S3 키를 저장해야 하므로 DB 커밋 **이전**에 일어나야 하고, 삭제는 롤백 가능성 때문에 DB 커밋 **이후**에 일어나야 합니다. 두 연산은 요구 시점이 정반대라 같은 방식으로 처리할 수 없었습니다.

**해결**

방향별로 보정 전략을 나눴습니다. 업로드는 실패 시 방금 올린 객체를 되돌리는 보상 삭제로, 삭제는 `TransactionSynchronization`의 `afterCommit` 훅으로 미뤄 처리했습니다.
...
보상 삭제 자체는 `deleteQuietly()`로 best-effort 처리했습니다. 여기서 예외를 다시 던지면 원래의 실패 원인이 가려지고, 최악의 경우에도 남는 것은 참조되지 않는 객체뿐이기 때문입니다.

함께 이미지와 일반 파일의 URL 정책도 분리했습니다. 본문에 계속 노출되는 이미지는 만료되지 않는 public URL을, 다운로드용 첨부는 TTL 5분 presigned URL을 내려줍니다.

**결과**

| 시나리오 | 개선 전 | 개선 후 |
|---|---|---|
| S3 업로드 성공 + DB 저장 실패 | S3에 고아 객체 잔존 | 보상 삭제로 정리 |
| DB 삭제 후 트랜잭션 롤백 | 레코드는 살아있고 파일만 손실 | S3 삭제가 커밋 뒤로 지연되어 파일 보존 |
| 보상 삭제 자체가 실패 | — | 원래 예외를 그대로 전파, 고아 객체만 잔존 |

### 4. PATCH 부분 수정에서 생략한 필드가 null로 덮어써지던 버그

**문제**

모임 수정은 `PATCH`로 일부 필드만 보내는 것을 전제로 설계했는데, 실제로는 **요청에 담지 않은 필드가 전부 `null`로 지워졌습니다.** 장소만 바꾸려고 `location`만 보내면 제목·설명·모임 일시가 함께 날아갔습니다.

**원인 분석**

요청 DTO가 record라 생략된 필드는 `null`로 역직렬화되는데, 엔티티의 `update()`가 전달받은 값을 무조건 대입하고 있었습니다. **"값을 보내지 않았다"와 "값을 null로 지워달라"를 구분하지 못한 것**이 원인이었습니다.

**해결**

엔티티의 `update()`에서 null이 아닌 필드만 반영하도록 바꿨습니다.
...
`voteDeadline`은 "null = 마감 없음"이라는 별도 의미가 있어 처음에는 무조건 반영했으나, **코드리뷰에서 다른 필드만 수정할 때 마감 시각이 사라진다는 지적**을 받고 이 필드에도 null 가드를 적용했습니다.

함께 모임 유형(`type`)은 수정 변경 불가로 막고 전용 에러코드를 추가했습니다. 유형이 바뀌면 참석 대상 집합 자체(전체 자동 포함 ↔ 신청 투표)가 달라져 이미 쌓인 투표 데이터의 의미가 깨지기 때문입니다.

**결과**

| 요청 | 개선 전 | 개선 후 |
|---|---|---|
| `{"location": "..."}` 만 전송 | title·description·gatheringDate가 null로 덮어써짐 | 보낸 필드만 반영, 나머지 유지 |
| `voteDeadline` 생략 | 마감 시간 손실 | 기존 값 유지 |
| `type` 변경 시도 | 그대로 반영되어 투표 대상 정합성 붕괴 | `400 GATHERING_TYPE_IMMUTABLE` 반환 |

### 5. 그룹 상태와 모집 게시글 상태의 동기화

**문제**

스터디·프로젝트 모집은 "게시글(Post + Study/Project)"과 "팀(Group + GroupMember)" 두 축으로 나뉘어 있습니다. 그런데 모집 상태 플래그가 `Group.recruitmentStatus`와 `Study.recruiting`/`Project.recruiting` **양쪽에 따로 존재**해, 한쪽만 바뀌면 목록에는 "모집중"인데 실제로는 신청이 막히는(또는 그 반대의) 불일치가 생겼습니다. 정원이 다 찼는데도 계속 모집중으로 노출되거나, 모집이 마감되는데 대기 중인 신청자가 아무 응답 없이 방치되는 문제도 함께 있었습니다.

**원인 분석**

상태 변경 진입점이 여러 곳(리더의 수동 마감, 멤버 승인으로 인한 정원 도달, 관리자 반려)이었는데 **각 진입점이 자기 쪽 플래그만 갱신**하고 있었습니다. 상태 전이를 한 곳으로 모으지 않으면 진입점이 늘어날 때마다 같은 버그가 재발하는 구조였습니다.

**해결**

모집 상태를 바꾸는 모든 경로가 `GroupService` 안에서 게시글 쪽 플래그까지 함께 갱신하도록 묶었습니다.
...
멤버 승인 경로에도 같은 동기화를 넣어, 현재 인원이 최대 인원에 도달하면 모집을 자동 마감하고 게시글 플래그까지 내리도록 했습니다. 최대 인원 수정 시에도 현재 인원보다 작은 값으로 줄이지 못하도록 검증을 추가했습니다.

**결과**

| 상황 | 개선 전 | 개선 후 |
|---|---|---|
| 리더가 모집 마감 | 그룹만 CLOSED, 게시글은 "모집중" 유지 | 게시글 `recruiting`도 함께 false |
| 정원 도달 (멤버 승인) | 수동으로 마감해야 함 | 자동 마감 + 게시글 동기화 |
| 마감 시 PENDING 신청자 | 무기한 대기 | 사유와 함께 일괄 REJECTED |
| 관리자 반려 후 재제출 | 별도 경로 없음 | 리더가 마감을 누르면 RESUBMITTED로 전환 |
| 최대 인원 축소 | 현재 인원보다 작게 설정 가능 | 400 + 현재 인원 안내 메시지 |

## 담당 범위

커밋 로그 기준 83개 커밋, 참여 기간 2025.12 ~ 2026.07입니다.
...
가장 많이 수정한 파일 상위 5개는 `ProjectService`(22회), `GroupService`(17회), `PostDTO`(16회), `PostMapper`(15회), `GroupController`(15회)입니다.

**담당하지 않은 영역** — 지원서 접수·심사·투표, 회원 인증·가입, 이메일 인증, 회비, 코딩테스트 문제, 자료실, 뉴스레터, 댓글·신고, 활동보고서, 배포 파이프라인·Nginx 설정.
