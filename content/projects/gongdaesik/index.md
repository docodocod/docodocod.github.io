...
### 1. 자바스크립트 이미지 뷰어에서 식단 데이터 추출

학교 학식 페이지는 iBook 전자책 뷰어라 HTML 소스에 식단 텍스트가 없고, 실제 식단은 JPG 이미지로만 존재해 `requests` + BeautifulSoup으로는 빈 `<div id="viewer">`만 얻었습니다. Playwright로 접속해 네트워크 요청을 관찰하니 식단표가 페이지별 이미지로 지연 로딩되고 URL은 매주 바뀌는 `bookcode`에 묶여 있었습니다. **브라우저 네트워크 요청을 인터셉트해 이미지 URL을 수집하고, 실패하면 스크린샷으로 폴백하는 2단 구조**로 만들었습니다 — 무인으로 돌려야 하는 파이프라인이라 한 경로가 막혀도 데이터가 비지 않는 쪽을 택했습니다.

한 URL 안에 E동·TIP 두 식당 식단이 섞여 들어오는데 **이미지 순서가 항상 같다는 보장이 없었습니다.** 순서에 의존하는 대신 Gemini로 식당을 먼저 분류하고, 분류가 실패할 때만 순서 기반 폴백을 쓰도록 했습니다. 식단 이미지에는 연도가 없고 `5/12(월)` 형태로만 적혀 있어 프롬프트에 기준 날짜를 주입해 연도 추정 오류를 막았습니다.

| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| 식단 데이터 확보 | 수동 입력 (정적 크롤링 불가) | 주 1회 자동 수집 |
| 수집 실패 대응 | 없음 | 스크린샷 폴백 + 재시도 3회 + 재크롤링 잡 2회 |
| 식당 분류 | 이미지 순서 의존 | Gemini 분류 + 순서 폴백 |

<details>
<summary>이미지 인터셉트와 폴백 코드</summary>
...
### 2. 로그는 "발송 성공"인데 알림이 오지 않던 문제

서버 로그에 `Push 발송 성공`이 찍히고 Prometheus `push_sent_total{status="success"}`도 증가하는데 **실제 단말에는 알림이 도착하지 않았습니다.** 파고들어 보니 세 가지가 겹쳐 있었습니다.

**메트릭 자체가 거짓이었습니다.** 발송 함수는 성공·만료·실패 세 가지를 반환하는데 성공 판정 조건이 잘못돼 **실패까지 전부 success 로 집계**되고 있었습니다. 여기에 클라우드 VM의 pause/resume으로 컨테이너 시계가 드리프트해 APScheduler가 의도한 08:00이 아닌 **한밤중에 발송**하고 있었고, 그 시각이면 단말이 Android Deep Doze 상태라 Web Push 기본값인 `Urgency: normal` 메시지가 지연·드롭될 수 있었습니다. 성공 판정을 3분기로 고치고, 호스트 시계를 컨테이너에 마운트하고, `Urgency: high`를 명시해 세 지점을 각각 막았습니다.

| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| 성공 메트릭 | 판정 조건 오류로 실패도 success | 성공 / 만료 / 실패 3분기 |
| 컨테이너 시각 | VM pause/resume 시 드리프트 | 호스트 시계 마운트 |
| Push 우선순위 | `Urgency: normal` (기본값) | `Urgency: high` |

<details>
<summary>세 지점 수정 코드</summary>
...
### 3. 3계층 캐시로 조회 지연과 반복 DB 부하 제거

식단은 주 1회 크롤링으로만 바뀌는데 메인 화면에 진입할 때마다 today·week 두 API가 매번 DB를 조회해 약 0.5초의 로딩 딜레이가 있었습니다. 서버 인메모리 TTL 캐시 + 프론트 localStorage SWR + HTTP `Cache-Control` 3계층으로 나누고 **데이터 성격에 맞춰 TTL을 달리했습니다** — 오늘 식단은 KST 자정까지만 캐시해 날짜가 바뀌면 자동 만료되게 했습니다.

캐시를 무조건 붙이면 **월요일 크롤링 전에 접속한 사용자의 "빈 식단"이 캐시에 굳어** 크롤링이 성공해도 만료 전까지 빈 화면이 나갑니다. **빈 결과는 캐시하지 않는** 조건을 걸고, 크롤링이 새 데이터를 저장하면 스케줄러가 식단 캐시를 프리픽스 단위로 무효화하도록 했습니다.

| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| 재방문 시 로딩 | 약 0.5초 딜레이 | localStorage 즉시 렌더 후 백그라운드 재검증 |
| 동일 요청의 DB 조회 | 매 요청 | 캐시 히트 시 0회 |
| 크롤링 전 빈 결과 | 자정까지 캐시에 고정 | 미캐싱 + 크롤링 후 무효화 |
| DB 커넥션 | pool_size 10 | pool_size 20 + overflow 20 |

<details>
<summary>캐시 조건과 프론트 SWR 코드</summary>
...
조회 경로의 인덱스도 함께 손봤습니다 — `meals(date, restaurant)`, `meals(week_start_date, restaurant)`, `users(notification_time)`, `semesters(start_date, end_date)`.

</details>

### 4. 알림 발송 병렬화

알림 발송이 구독 하나씩 순차로 이뤄졌고 `pywebpush`가 동기 HTTP 라이브러리라 **발송 중 이벤트 루프가 통째로 블로킹**됐습니다. 병목은 CPU가 아니라 FCM/APNs 왕복 대기였는데, 그렇다고 전부 동시에 던지면 외부 푸시 서버에 부담이 가므로 **동시성 상한을 둔 병렬화**를 택했습니다. 동기 발송을 `asyncio.to_thread`로 감싸 루프 블로킹을 없애고 `Semaphore(20)`으로 동시 발송 수를 제한했습니다.

앱 삭제·권한 취소로 만료된 구독(410 Gone)이 DB에 남아 매일 실패 요청을 반복하고 있었는데, `pywebpush`가 410을 전달하는 방식이 일반적이지 않아 감지 조건이 헐려 있었습니다. `e.response`가 `None`인 경우까지 커버하도록 고치고, 발송 결과에서 410을 모아 한 번의 쿼리로 일괄 삭제합니다.

| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| 발송 방식 | 구독별 순차 | `Semaphore(20)` 상한 병렬 |
| 이벤트 루프 | 발송 내내 블로킹 | `to_thread` 분리 |
| 전체 발송 시간 | 구독 수에 선형 비례 | 구독 수 ÷ 20 에 비례 |
| 만료 구독 | 미감지로 DB 잔존 | 일괄 DELETE |

<details>
<summary>병렬 발송과 410 처리 코드</summary>
...
### 5. UTC 서버에서 "어제 식단"이 보이던 버그

배포 후 오전에 앱을 열면 전날 식단이 표시됐고, 로컬(KST) 개발 환경에서는 재현되지 않았습니다. 서버 컨테이너가 UTC로 동작하고 `date.today()`가 UTC 기준 날짜를 반환하는데, **KST 09:00 이전에는 UTC 기준으로 아직 전날**이고 사용자가 아침에 접속하는 시간대가 정확히 그 구간이었습니다. 알림 스케줄러도 같은 기준을 써서 발송 대상 날짜가 함께 어긋났습니다.

서버 로컬 타임존과 무관하게 KST 날짜를 반환하는 헬퍼를 만들고 **날짜를 다루는 호출부를 전부 교체했습니다** — 한 곳만 고치면 다음에 같은 버그가 다시 난다고 봤습니다. APScheduler는 `timezone="Asia/Seoul"`로 고정하고, 캐시 TTL도 KST 자정 기준으로 계산합니다.
...
| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| 오전 조회 결과 | KST 00:00~09:00 에 전날 식단 | 항상 KST 기준 당일 |
| 기준 시각 | `date.today()` (서버 = UTC) | `today_kst()` + 스케줄러 타임존 고정 |
