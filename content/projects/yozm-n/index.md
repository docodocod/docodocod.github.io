...
### 1. 추천 인덱싱의 외부 API 호출 N회 → 1회

초기 추천 로직은 DB의 전체 기사를 대상으로, 기사 1건마다 임베딩 API를 1회 그리고 OpenSearch 인덱싱을 1회씩 호출했습니다. 인덱싱 대상이 삭제되지 않은 전체 기사여서 **기사가 쌓일수록 호출 횟수가 선형으로 늘어나는 구조**였습니다.

파고들어 보니 세 가지가 겹쳐 있었습니다. 임베딩 API를 기사 단위로 호출하고 있었는데 **OpenAI 임베딩 API는 `input`에 배열을 받아 배치가 가능**했고, OpenSearch 인덱싱이 동기 클라이언트라 `asyncio.gather`로 감싸도 실제로는 이벤트 루프를 블로킹했으며, 추천은 사용자의 관심 언론사·카테고리 안에서만 의미가 있는데 전체 기사를 인덱싱하고 있었습니다. 배치 임베딩 + `_bulk` API + 인덱싱 범위 축소를 함께 적용했습니다 — **최적화에 들어가기 전에 그 API가 배치를 지원하는지부터 확인한 것**이 셋 중 가장 큰 이득이었습니다.

| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| 기사 N건 인덱싱 시 임베딩 API 호출 | N회 | **1회** |
| 기사 N건 인덱싱 시 OpenSearch 요청 | N회 (동기, 루프 블로킹) | **1회** (`_bulk`, 비동기) |
| 인덱싱 대상 | 삭제되지 않은 전체 기사 | 사용자 선호 언론사·카테고리의 당일 기사 |
| k-NN 검색 클라이언트 | 동기 `opensearch-py` | 비동기 `aiohttp` |

<details>
<summary>개선 전후 코드</summary>
...
### 2. Celery 큐 분리로 후처리 장애 격리

초기에는 Celery 워커 컨테이너 하나가 모든 태스크를 처리했습니다. 기사 저장 직후 실행되는 후처리는 성격이 서로 다릅니다 — TTS 생성은 Google Cloud API 응답을 기다리는 **I/O 대기** 작업이고, 썸네일 생성은 Pillow로 디코딩·리사이즈·인코딩하는 **CPU 작업**입니다. 한 종류가 몰리면 그쪽이 워커 슬롯을 점유하는 동안 다른 쪽 태스크가 밀렸습니다.

크롤링 1사이클에서 기사가 저장될 때마다 TTS와 이미지 태스크가 각각 발행돼, 26개 피드 × 3건 = **최대 156개 태스크가 하나의 큐로 몰리는** 구조였고 워커가 하나뿐이라 처리 순서를 제어할 방법이 없었습니다. Kombu `Queue`로 큐를 분리하고 워커 컨테이너를 큐별로 나눠 **작업 성격에 맞게 concurrency를 다르게** 줬습니다(I/O 위주 TTS 8, CPU 위주 이미지 6).

| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| 워커 컨테이너 | 1개 (전체 큐 담당) | 2개 (`tts`, `image`) |
| 큐 | 기본 큐 1개 | `tts` / `image` / `default` |
| concurrency | 8 (공통) | TTS 8 (I/O), 이미지 6 (CPU) |
| 장애 격리 | 없음 | 한쪽 장애가 다른 큐에 전파되지 않음 |
| 관측 | — | Flower로 큐별 태스크 현황 |

<details>
<summary>큐 분리 설정</summary>
...
### 3. 후처리 미완료 기사가 API로 노출되던 문제

기사는 DB에 저장되는 순간 음성·썸네일 URL이 모두 빈 문자열이고, 실제 값은 Celery 워커가 만든 다음에야 채워집니다. 그런데 최신 기사 목록 API는 발행일 내림차순 정렬이라 **가장 최근 기사 = 후처리가 아직 끝나지 않은 기사**가 됐고, 앱 첫 화면에 이미지가 깨지고 재생이 되지 않는 기사가 그대로 올라왔습니다.

쓰기 경로와 읽기 경로가 분리된 최종적 일관성 구조인데 **읽기 쪽에 "후처리 완료" 조건이 없다는 것**이 원인이었습니다. 워커 완료를 동기적으로 기다리면 비동기로 분리한 의미가 사라지므로, 별도 상태 컬럼 없이 판별할 방법이 필요했습니다. **후처리의 산출물인 URL 컬럼 자체가 완료 플래그 역할을 할 수 있다**는 점을 이용해 조회 쿼리에 조건을 넣었습니다.

썸네일이 필터링 대상이 되면 원본 이미지가 없는 기사는 영구 제외되므로, 워커에서 원본이 없거나 다운로드에 실패하면 GCS의 fallback 이미지 URL을 채우도록 보완했습니다.

| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| 최신 기사 목록 조건 | 삭제 여부만 확인 | + 음성 2종·썸네일이 모두 채워진 기사만 |
| 기사 상세 조건 | ID 일치만 | + 삭제 여부와 후처리 완료 |
| 원본 이미지 없는 기사 | 영구 제외 | fallback 이미지로 정상 노출 |

<details>
<summary>조회 쿼리 조건</summary>
...
### 4. 전역 JWT 미들웨어가 CORS Preflight를 막던 문제

인증을 라우터별 `Depends`가 아닌 전역 `BaseHTTPMiddleware`로 처리하는 구조였는데, 브라우저에서 호출하면 실제 요청 전에 나가는 `OPTIONS` preflight가 미들웨어에 걸렸습니다. preflight에는 `Authorization` 헤더가 실리지 않으므로 401이 반환됐고, 브라우저는 본 요청을 아예 보내지 않아 **보호된 API가 모두 CORS 오류로 실패**했습니다.

`app.add_middleware()`로 등록한 미들웨어는 나중에 등록한 것이 바깥쪽에 쌓입니다. `AuthMiddleware`가 `CORSMiddleware`보다 뒤에 등록돼 **preflight가 CORS 처리에 닿기 전에 인증 검사를 먼저 통과해야 했습니다.** 미들웨어 최상단에서 `OPTIONS`를 인증 없이 통과시키고, `allow_origins=["*"]`와 `allow_credentials=True`를 함께 쓸 수 없는 문제도 있어 허용 오리진을 명시했습니다.

| 항목 | 개선 전 | 개선 후 |
|---|---|---|
| `OPTIONS` preflight | 401 (Authorization 헤더 없음) | 인증 검사 없이 통과 |
| `allow_origins` | `["*"]` (credentials 와 충돌) | 개발·배포 오리진 4개 명시 |
| 토큰 타입 검증 | 서명·만료만 | `access` / `refresh` 를 경로별 구분 |

<details>
<summary>미들웨어 처리 코드</summary>
...
### 5. 변경 주기가 다른 데이터의 계층별 캐싱

성격이 다른 세 종류의 반복 비용이 있었습니다. 실시간 뉴스 목록은 크롤링 주기가 15분인데도 진입할 때마다 DB를 조회했고, 관심 설정은 거의 바뀌지 않는데 화면마다 조회 쿼리가 나갔으며, 추천 API는 호출할 때마다 사용자 키워드를 OpenAI 임베딩 API로 다시 변환했습니다.

**세 데이터의 변경 주기가 서로 달라 하나의 TTL로 묶을 수 없었습니다.** 기사 목록은 15분마다 갱신되지만 조회는 훨씬 잦고, 사용자 설정은 사용자가 수정할 때만 바뀌며, 키워드 임베딩은 같은 입력에 항상 같은 결과가 나오는 순수 함수입니다. 계층별로 다른 TTL을 두되, **사용자 설정만은 TTL이 아니라 쓰기 시점 무효화**로 정합성을 확보했습니다 — 설정은 바뀐 즉시 반영돼야 하는 데이터라서입니다.

| 캐시 계층 | TTL | 무효화 방식 |
|---|---|---|
| 실시간 뉴스 목록 | 60초 | TTL 만료 (크롤링 주기보다 짧게) |
| 사용자 관심 설정 4종 | 24시간 | 설정 수정 API에서 해당 키 즉시 삭제 |
| 키워드 임베딩 | 24시간 | TTL 만료 (동일 입력 → 동일 결과) |
| 챗봇 대화 히스토리 | 30분 | 갱신 시 `keepttl=True` 로 TTL 유지 |

<details>
<summary>계층별 캐시 코드</summary>
...
5명 팀에서 **팀장 / 백엔드 / DevOps**를 맡았고, 아래가 제가 설계·구현한 영역입니다.
